...
export function UpgradeModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='max-w-3xl bg-white/90 dark:bg-black/[0.98] border-0 p-0 overflow-hidden'>
        {/* Sparkles background */}
        ...
        <div className='relative z-20 p-8'>
          <DialogHeader className='mb-8 text-center'>
            <div className='flex justify-center items-center mb-2'>
              <DialogTitle className='text-4xl text-center font-extrabold tracking-wide bg-gradient-to-r from-purple-400 via-blue-400 to-purple-600 dark:from-white dark:to-gray-400 bg-clip-text text-transparent'>
                Upgrade Your Plan
              </DialogTitle>
            </div>
            {/* {limitType ? (
              <p className='text-gray-600 text-center dark:text-gray-400 mt-2'>
                Your free plan is limited to {limit} {limitType}. You currently
                have {currentCount}/{limit}.
              </p>
            ) : (
              <p className='text-gray-600 text-center dark:text-gray-400 mt-2'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
              ...
              >
                ...
                <ul className='space-y-3 mb-8'>
                  {proPlan.features.map((feature) => (
                    <li
                      key={feature.label}
                      className='flex items-center text-lg justify-between text-gray-700 dark:text-gray-300 py-1'
                    >
                ...
                <Link href='/pricing' className='w-full'>
                  <Button className='w-full bg-purple-600 hover:bg-purple-700 h-12 text-lg text-white'>
                    Upgrade to Pro
                  </Button>
                </Link>
            ...
            {lifetimePlan && (
              <div className='rounded-xl p-8 relative overflow-hidden border border-gray-200 dark:border-white/[0.08] bg-gray-100 shadow-md dark:bg-black/80'>
                <h3 className='text-2xl font-semibold text-gray-900 dark:text-white mb-4 text-center'>
                  {lifetimePlan.name}
                </h3>
                <div className='flex justify-center items-end mb-6'>
                ...
                <ul className='space-y-3 mb-8'>
                  {lifetimePlan.features.map((feature) => (
                    <li
                      key={feature.label}
                      className='flex items-center justify-between text-lg text-gray-700 dark:text-gray-300 py-1'
                    >
                      <div className='flex items-center'>
                        {/* <Check className="w-4 h-4 text-purple-400 mr-2" /> */}
                        <span>{feature.label}</span>
                ...
                <Link href='/pricing' className='w-full'>
                  <Button className='w-full bg-gray-200 hover:bg-gray-300 text-gray-900 dark:bg-white/10 dark:hover:bg-white/20 dark:text-white h-12 text-lg'>
                    Go Lifetime
                  </Button>
                </Link>
              </div>
